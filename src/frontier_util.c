...
void ClearRankingHallRecords(void)
{
    ...
    for (i = 0; i < HALL_FACILITIES_COUNT; i++)
    {
        for (j = 0; j < 2; j++)
        {
            for (k = 0; k < 3; k++)
            {
                CopyTrainerId(gSaveBlock2Ptr->hallRecords1P[i][j][k].id, NULL);
    ...
    for (j = 0; j < 2; j++)
    {
        for (k = 0; k < 3; k++)
        {
            CopyTrainerId(gSaveBlock2Ptr->hallRecords2P[j][k].id1, NULL);
            CopyTrainerId(gSaveBlock2Ptr->hallRecords2P[j][k].id2, NULL);
